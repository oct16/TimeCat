...
(function (FormElementEvent) {
    ...
    FormElementEvent["CHANGE"] = "CHANGE";
...
var ChildListUpdateDataType;
(function (ChildListUpdateDataType) {
    ChildListUpdateDataType["ADD"] = "ADD";
    ChildListUpdateDataType["DELETE"] = "DELETE";
    ChildListUpdateDataType["MOVE"] = "MOVE";
})(ChildListUpdateDataType || (ChildListUpdateDataType = {}));
                            ...
                            if (node.nodeType === Node.TEXT_NODE) {
                                ...
                                joinData({
                                    ...
                                    pos: parent.childNodes.length > 0
                                        ? [...parent.childNodes].indexOf(node)
                                        : null
                                });
                            }
                        });
...
function recordAll(emitter) {
    const recordTasks = [...Object.values(snapshots)];
...
class Container {
    ...
    initSandbox() {
        ...
        sandBoxDoc.replaceChild(convertVNode(this.vNode), sandBoxDoc.documentElement);
    }
    initTemplate() {
    ...
    switch (type) {
        ...
        case SnapshotType.DOM_UPDATE:
            ...
            mutations.forEach((mutate) => {
                ...
                const { value, attr, type, parentId, pos, nodeId, vNode } = data;
                switch (mType) {
                    case 'attributes':
                    ...
                    case 'childList':
                        ...
                        const targetNode = nodeStore.getNode(nodeId) || convertVNode(vNode);
...
class PlayerComponent {
    constructor(data, pointer, progress) {
        this.speed = 0;
        this.index = 0;
        this.frameIndex = 0;
...
class ProgressComponent {
    constructor(c) {
        ...
        this.progress = c.querySelector('.wr-progress');
        this.thumb = this.progress.querySelector('.wr-thumb');
        this.slider = this.progress.querySelector('.wr-slider-bar');
    }
...
export { DBPromise as DB, record, replay };
